...
- **`claude_tokens_spent`** — Claude tokens actually used on this task: routing/adapting overhead (usually small), any escalation execution, any Claude review. Claude-route tasks spend ≈ everything, so `avoided ≈ 0` — that's honest and keeps the metric credible.
